...
|---|---|---|---|---|
| `wedge_behavior/` | Load-bearing shift in agent responses when `frame_check` is invoked vs. not | Pipeline smoke test executed 2026-05-12 (data kept private, not in this repo); main study under PROTOCOL_v2 pending external-source documents + independent raters | [`PROTOCOL_v1.md`](wedge_behavior/PROTOCOL_v1.md), [`STATUS.md`](wedge_behavior/STATUS.md) | [`run_pilot.py`](wedge_behavior/run_pilot.py) + [`run_arms.py`](wedge_behavior/run_arms.py) |
...
| (helpers) | External-document sourcing for the main study | Command-line runnable; standardizes URL + retrieval timestamp + SHA capture and validates inclusion criteria | — | [`external_doc_sourcer.py`](external_doc_sourcer.py) |
...
`external_doc_sourcer.py` is the command-line helper for sourcing main-study documents from outside the author's reach. Takes URL + text, validates against PROTOCOL inclusion criteria, emits structured corpus entry with provenance metadata (URL, SHA, retrieval timestamp, word count). Removes the self-authored-document confound at sourcing time.
