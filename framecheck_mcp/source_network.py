...
_HEADERS = {"User-Agent": "FrameCheck/1.0 (+https://github.com/Clarethium/frame-check)"}
_TIMEOUT = 5
...
class _ProviderHealth:
    ...
    def snapshot(self) -> dict:
        ...
        Shape includes by_kind breakdown for actionable signal
        (rate_limited / auth / server_error / deadline / other).
        Mirror of source_network.py (root); see that module for the
        full rationale.
        """
        now = time.time()
        with self._lock:
            ...
            for provider, events in self._events.items():
                ...
                # returned shape matches the docstring contract.
                # Mirror of source_network.py (root).
                by_kind: dict = {
                    "rate_limited": 0,
                    "auth": 0,
                    "server_error": 0,
                ...
                providers_out[provider] = {
                    "total": total,
                    "by_kind": by_kind,
                    # Backward-compat alias for pre-2026-05-03
                    # consumers that branched on rate_limited directly.
...
# urllib's per-op timeout).
_FETCH_JSON_DEADLINE_SECONDS = float(
    os.environ.get("SN_FETCH_JSON_DEADLINE_SECONDS", "8")
)


def _urlopen_with_deadline(req, per_op_timeout, total_deadline=None):
    """urlopen with HARD caller-side total deadline + per-op timeout.

    Returns response BYTES; raises _FuturesTimeoutError on caller-side
    ...
    try:
        ...
        try:
            ...
            # Defensive diagnostics; mirror of source_network.py.
...
def _fetch_json(url, total_deadline=None):
    ...
    Mirror of source_network.py (root). Returns parsed JSON dict on
    success, None on any error including caller-side deadline
    exhaustion. The total_deadline parameter (default
    _FETCH_JSON_DEADLINE_SECONDS, env-overridable via
    SN_FETCH_JSON_DEADLINE_SECONDS) caps the caller-side wait above
    the per-socket-operation _TIMEOUT.
    """
    deadline = (
        total_deadline if total_deadline is not None
        else _FETCH_JSON_DEADLINE_SECONDS
...
# ── REST Countries ──
#
# The country regexes and canonical map live in entity_data.py
# (top-level module, shared with entity_classifier). Re-imported here
# so existing reads via this module's namespace keep resolving.
...
def verify_wolfram(decomp):
    ...
    try:
        req = urllib.request.Request(url, headers=_HEADERS)
        # Caller-side deadline + per-op timeout (mirror).
...
_SEC_HEADERS = {"User-Agent": "FrameCheck/1.0 (+https://github.com/Clarethium/frame-check)"}
...
def _get_sec_tickers():
    ...
    with _SEC_TICKERS_LOCK:
        ...
        try:
            ...
            # Caller-side deadline + per-op timeout (mirror).
            data = json.loads(_urlopen_with_deadline(req, per_op_timeout=10))
...
def verify_claims_source_network(
    ...
    # Cooperative cancellation: capture the SN deadline origin HERE so
    # workers can poll the deadline without depending on the
    # as_completed loop yielding. Mirror of source_network.py (root);
    # see that module for the full architectural rationale.
    sn_orchestrator_start = time.monotonic()

    def _budget_exhausted_in_worker():
        """Return True when SN_BUDGET_SECONDS deadline has elapsed."""
    ...
    def _budget_marked_result(claim, decomp):
        """Synthesize the unverifiable-with-budget-marker result.
        decomp is REQUIRED (app.py:2222 reads decomp.subject
        unconditionally; None would crash). Mirror of source_network.py
        (root)."""
    ...
    def _verify_one(claim):
        ...
        for fn in source_fns:
            # Per-provider budget poll for symmetric coverage at every
            # provider boundary. Mirror of source_network.py (root).
            if _budget_exhausted_in_worker():
                return _budget_marked_result(claim, decomp=decomp)
        ...
        # Budget poll BEFORE the non-Wikipedia sub-pool. Mirror.
