...
**Status:** v1, single-curator. Withdrawn from v1 publication per [INDEX.md](https://github.com/lluvr/frame-check-mcp/blob/master/data/frame_library/INDEX.md) "v1 publication state" table (bilateral-reinforcement mechanism not grounded in cited experimental evidence). Source markdown preserved for citation continuity; the FVS-004 ID is reserved and will not be reused.
