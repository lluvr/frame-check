...
**Engine-emit disclosure.** FVS-020 is excluded from V4.2 engine emission per Step 4 ratification (`vocabulary_only` status; see [INDEX.md](https://github.com/lluvr/frame-check-mcp/blob/master/data/frame_library/INDEX.md)). The frame is part of the FVS vocabulary but the engine does not emit a `library_consensus_ac1` for it. See `meta.fvs_020_status` in V4.2 results.
