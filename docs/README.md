...
1. [ANTICIPATED_CRITIQUES.md](ANTICIPATED_CRITIQUES.md): self-enumerated
   adversarial readings of the project's claims.
2. [VALIDATION_PROGRAM.md](VALIDATION_PROGRAM.md): the validation program
   (observational + formal plans).

### "I want to understand frame divergence"

1. [FRAME_DIVERGENCE_CONTRACT_v1.md](FRAME_DIVERGENCE_CONTRACT_v1.md): the wire
   contract for the `divergence` block, MCP resource URIs, error envelope,
   versioning commitments (c1.0 shipping).
...
[`../data/worked_examples/`](../data/worked_examples/): published worked
examples with multi-LLM comparisons + per-document Frame Check analysis.
Each has a `.md` writeup and a directory carrying the underlying JSON
artifacts (LLM responses, frame_check results, pairwise comparisons).
...
- [FRAME_DIVERGENCE_CONTRACT_v1.md](FRAME_DIVERGENCE_CONTRACT_v1.md): the
  divergence-block wire contract (c1.0 shipping).
