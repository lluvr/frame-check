...
def main() -> int:
    ...
    failures: list[str] = []
    for path in files:
        # corpus_site/ is the generated static site. Its pages render
        # analyzed documents (em-dashes appear there as data) and
        # pre-discipline worked-example writeups. It is build output, not
        # authored project prose, so the prose em-dash rule does not apply.
