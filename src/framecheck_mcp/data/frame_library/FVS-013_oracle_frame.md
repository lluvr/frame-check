...
**When this frame is appropriate:** NEVER as a default. Oracle mode is the failure state this catalog flags as the most dangerous. There are contexts where deferring to AI is reasonable (calculation, formatting, translation), but even these benefit from spot-checking. Oracle mode as a deliberate choice (after evaluating) is different from oracle mode as an unconscious default.
