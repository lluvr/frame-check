...
- Construct-validity audit v1.1: 13 constructs audited through 5 evaluator questions (v1.0 + V4.2 LLM-judge FVS detection added 2026-04-27); 5 known residual cases enumerated; cross-surface alignment matrix (template / portrait / MCP / comparison / LLM context) per construct; L5 framework gap surfaced on construct #13 (V4.2 emissions fit none of the four existing claim levels cleanly) with two resolution proposals named for v1.2.
...
#### §4.1 The four claim levels

Tabulate the four claim_level values with epistemic status and treatment:
...
| `classifier_output` | Decision over surfaces with confidence + runner-up | Restate per construct.how_to_serialize; surface confidence and runner-up explicitly. |
...
Each construct emits a `construct` block with three keys. signal_type names the construct's shape (vocabulary_and_pattern_detector / cascade_classification / distribution_with_dominant / scoring_with_runner_up / composed_dimension). statement defines the construct in operational terms and names what the construct does NOT measure. how_to_serialize instructs the agent on correct restatement form. The construct-honesty mechanism unifies the 12 deterministic substrate constructs under a single discipline; CONSTRUCT_VALIDITY_AUDIT_v1.md (v1.1) is the canonical per-construct walk and includes the 13th construct (V4.2 LLM-judge FVS detection, evaluation-engine-only) that exposes the L5 framework gap (no signal_type fits the V4.2 binary-without-confidence-or-runner-up shape under the current four-claim-level table).

#### §4.3 Per-claim-level worked examples

Three worked examples, one per level (skip agent_generated; nondeterministic):
...
frame_library_matches (20 FVS frames at detector_measurement); frame_deepening (3 sub-constructs at detector_measurement); frame_patterns (8 patterns at composed_pattern); decision_readiness_dims (5 dimensions at composed_pattern); absence_clusters (composed from frame matches and dimensions); frame_opportunities (opt-in agent_generated). Each gets a paragraph with construct, claim_level, what-it-measures, what-it-does-NOT-measure. CONSTRUCT_VALIDITY_AUDIT_v1.md is the canonical reference.
...
**F1. L5 framework table.** Replaces v1's "construct-honesty posture across five signals" with the four-claim-level table from CONSTRUCT_VALIDITY_AUDIT_v1.md. Shows the framework spine. Drafted 2026-04-27 in `METHODOLOGY_PAPER_v2_FIGURES_v1.md`; includes the L5 framework gap on construct #13 (V4.2 LLM-judge) as a load-bearing footer per audit v1.1.
...
- **Construct-validity audit v1 covers 12 substrate constructs; v1.1 adds construct #13 (V4.2 LLM-judge FVS detection) and surfaces an L5 framework gap**. The audit's version-tracking discipline is operationally proven (v1.0 -> v1.1 increment 2026-04-27). The paper either snapshots v1.1 or commits to v-tracking discipline going forward. Curator-gated. The L5 framework gap on construct #13 is a load-bearing finding, not a gap to paper over: the audit's Proposal A (fifth claim level `llm_classifier_output`) and Proposal B (broaden `classifier_output`) are operator-decision items for v1.2; paper §4 should frame the gap as evidence the framework is honest about its own limits.
