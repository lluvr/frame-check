...
**Status:** v1, single-curator. Withdrawn from v1 publication per [INDEX.md](https://github.com/lluvr/frame-check-mcp/blob/master/data/frame_library/INDEX.md) "v1 publication state" table (superseded by FVS-005, which carries the same four-layer attribution mechanism with broader scope). Source markdown preserved for citation continuity; the FVS-003 ID is reserved and will not be reused.
